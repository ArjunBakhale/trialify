...
import { z } from "zod";
import type { StructuredPatientProfile } from "./emrAnalysisAgent";
import { TrialScoutResponse } from "./trialScoutAgent";
import { EligibilityScreenerResponse } from "./eligibilityScreenerAgent";
 ...
 * - Structured Output Validation: Uses Zod schemas for final reports
 * - Patient-Centric Accuracy: Maintains traceability of all recommendations
 * - Operational Observability: Provides comprehensive reporting
 */
...
// Clinical Report Schema (from design principles)
...
// Export the schema as well for validation
export { ClinicalReportSchema };
...
const reportGeneratorTool = {
  ...
  execute: async (ctx: { context: SummarizationInput }) => {
    const { patientProfile, trialScoutResults, eligibilityResults } = ctx.context;
...
/**
 * Utility function to run summarization with error handling
 */
export async function generateClinicalReport(
  patientProfile: StructuredPatientProfile,
  trialScoutResults: TrialScoutResponse,
  eligibilityResults: EligibilityScreenerResponse
  ...
  try {
    ...
    const summarizationInput = {
      ...
      trialScoutResults,
      eligibilityResults,
    } satisfies SummarizationInput;

    const report = await reportGeneratorTool.execute({
      context: summarizationInput,
    });

    const executionTime = Date.now() - startTime;
    console.log(`✅ Clinical report generated successfully in ${executionTime}ms`);
    return report;
  } catch (error) {
...
/**
 * Human-in-the-loop suspension point for physician review
 */
export async function suspendForPhysicianReview(
  eligibilityResults: EligibilityScreenerResponse,
  message: string = "Please review eligibility results before final report generation"
): Promise<void> {
  console.log("⏸️ Suspending workflow for physician review...");
  console.log(`📋 Review Message: ${message}`);
  console.log(`📊 Eligibility Summary:`, eligibilityResults.summary);
  
  // In a real implementation, this would integrate with Mastra's .suspend() functionality
  // For now, we'll log the suspension point
  console.log("👨‍⚕️ Physician review checkpoint reached");
  console.log("🔄 Workflow will resume after physician approval");
}